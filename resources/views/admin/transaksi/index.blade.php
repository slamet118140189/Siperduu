    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="d-flex justify-content-between mb-3">
                ...
                <a href="{{ route('admin.transaksi.create') }}" class="btn-sm btn-primary">Tambah Transaksi</a>
            </div>
            <!-- Tambahkan tombol "Jatuh Tempo" -->
            <button id="btnPeminjamHampirJatuhTempo" class="btn btn-warning">Peminjam Hampir Jatuh Tempo</button>
                ...
                <table id="example1" class="table table-bordered table-striped">
                    ...
                    <tbody>
                        @foreach ($transaksis as $transaksi)
                            <tr>
                                {{dd($transaksi->tanggal_pengembalian)}}
                                <td>{{ $loop->iteration }}</td>
                                <td>{{ $transaksi->kode_pinjam }}</td>
                                <td>{{ $transaksi->peminjam_id }}</td>
                                <td>{{ $transaksi->petugas_pinjam }}</td>
                                <td>{{ $transaksi->petugas_kembali }}</td>
                                <td style="background-color: {{ $transaksi->statusColor }}">{{ $transaksi->status }}</td>
                                <td>{{ $transaksi->denda }}</td>
                                <td>{{ $transaksi->tanggal_pinjam }}</td>
                                <td>{{ $transaksi->tanggal_kembali }}</td>
                                <td>{{ $transaksi->tanggal_pengembalian ? \Carbon\Carbon::parse($transaksi->tanggal_pengembalian)->format('d-M-Y') : '-' }}</td>
                                <td class="manage-row">
...
@section('script')
    ...
    <script>
        $(function() {
            $('#btnPeminjamHampirJatuhTempo').on('click', function() {
                $.ajax({
                    url: '/admin/peminjam-hampir-jatuh-tempo',
                    type: 'GET',
                    dataType: 'json',
